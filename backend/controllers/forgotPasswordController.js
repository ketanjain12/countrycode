...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ status: true, msg: 'Reset code sent to email' });
  } catch (error) {
    console.error(error);
...
exports.verifyCode = async (req, res) => {
  ...
  try {
    ...
    if (!user || user.resetCode !== code || Date.now() > user.resetCodeExpiry) {
      return res.status(400).json({ status: false, msg: 'Invalid or expired reset code' });
    }

    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '15m' });

    res.status(200).json({ status: true, msg: 'Code verified', token });
  } catch (error) {
    console.error(error);
